...
namespace cryptonote
{
  ...
  bool check_inputs_types_supported(const transaction& tx)
  {
    BOOST_FOREACH(const auto& in, tx.vin)
    {
      if (tx.blob_type == BLOB_TYPE_CRYPTONOTE_AIMONIT) {
        CHECK_AND_ASSERT_MES(in.type() == typeid(txin_aimonit_key), false, "wrong variant type: "
          << in.type().name() << ", expected " << typeid(txin_aimonit_key).name()
  ...
  bool get_transaction_hash(const transaction& t, crypto::hash& res, size_t* blob_size)
  {
    ...
    // base rct
    {
      ...
      const size_t outputs = t.blob_type == BLOB_TYPE_CRYPTONOTE_AIMONIT ? t.vout_aimonit.size() : t.blob_type != BLOB_TYPE_CRYPTONOTE_XHV ? t.vout.size() : t.vout_xhv.size();
    ...
    else
    {
      ...
      const size_t outputs = t.blob_type == BLOB_TYPE_CRYPTONOTE_AIMONIT ? t.vout_aimonit.size() : t.blob_type != BLOB_TYPE_CRYPTONOTE_XHV ? t.vout.size() : t.vout_xhv.size();
      size_t mixin;
      if (t.blob_type == BLOB_TYPE_CRYPTONOTE_AIMONIT) {
        mixin = t.vin.empty() ? 0 : t.vin[0].type() == typeid(txin_aimonit_key) ? boost::get<txin_aimonit_key>(t.vin[0]).key_offsets.size() - 1 : 0;
