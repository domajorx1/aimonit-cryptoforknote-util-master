...
#include "serialization/aimonit_pricing_record.h"
...
namespace cryptonote
{
  ...
  // AIMONIT
  struct txout_aimonit_tagged_key
  {
    txout_aimonit_tagged_key() { }
    txout_aimonit_tagged_key(const crypto::public_key &_key, const std::string &_asset_type, const crypto::view_tag &_view_tag) : key(_key), asset_type(_asset_type), view_tag(_view_tag) { }
    crypto::public_key key;
    std::string asset_type;
    crypto::view_tag view_tag; // optimization to reduce scanning time
  ...
  typedef boost::variant<txin_gen, txin_to_script, txin_to_scripthash, txin_to_key, txin_offshore, txin_onshore, txin_xasset, txin_aimonit_key> txin_v;
  ...
  typedef boost::variant<txout_to_script, txout_to_scripthash, txout_aimonit_tagged_key> txout_stablero_target_v;
  ...
  public:
    ...
    std::vector<tx_out_aimonit> vout_aimonit;
    //extra
    ...
    BEGIN_SERIALIZE()
      ...
      if (blob_type == BLOB_TYPE_CRYPTONOTE_AIMONIT)
        FIELD(vout_aimonit)
  ...
  public:
    ...
    BEGIN_SERIALIZE_OBJECT()
      ...
      else
      {
        ...
        {
          ar.begin_object();
          bool r = rct_signatures.serialize_rctsig_base(ar, vin.size(), blob_type == BLOB_TYPE_CRYPTONOTE_AIMONIT ? vout_aimonit.size() : blob_type != BLOB_TYPE_CRYPTONOTE_XHV ? vout.size() : vout_xhv.size());
          ...
          if (rct_signatures.type != rct::RCTTypeNull)
          {
            ...
            ar.begin_object();
            if (blob_type == BLOB_TYPE_CRYPTONOTE_AIMONIT) {
              r = rct_signatures.p.serialize_rctsig_prunable(ar, rct_signatures.type, vin.size(), vout_aimonit.size(),
                  vin[0].type() == typeid(txin_aimonit_key) ? boost::get<txin_aimonit_key>(vin[0]).key_offsets.size() - 1 : 0);
            } else if (blob_type == BLOB_TYPE_CRYPTONOTE_XHV) {
  ...
  size_t transaction::get_signature_size(const txin_v& tx_in)
  {
    struct txin_signature_size_visitor : public boost::static_visitor<size_t>
    {
      ...
      size_t operator()(const txin_aimonit_key& txin) const {return txin.key_offsets.size();}
    };
  ...
  struct block_header
  {
    ...
    aimonit_oracle::pricing_record aimonit_pricing_record;
    crypto::cycle cycle;
    ...
    BEGIN_SERIALIZE()
      ...
      if (blob_type == BLOB_TYPE_CRYPTONOTE_AIMONIT) {
        if (major_version >= 6)
        {
          FIELD_N("pricing_record", aimonit_pricing_record)
        }
        else if (major_version >= 4)
        {
          ...
          {
            FIELD(pr_v3)
            pr_v3.write_to_pr(aimonit_pricing_record);
          }
          else
          {
            pr_v3.read_from_pr(aimonit_pricing_record);
            FIELD(pr_v3)
          }
        }
        else if (major_version >= 3)
        {
          ...
          {
            FIELD(pr_v2)
            pr_v2.write_to_pr(aimonit_pricing_record);
          }
          else
          {
            pr_v2.read_from_pr(aimonit_pricing_record);
            FIELD(pr_v2)
          }
        }
        else
        {
          ...
          {
            FIELD(pr_v1)
            pr_v1.write_to_pr(aimonit_pricing_record);
          }
          else
          {
            pr_v1.read_from_pr(aimonit_pricing_record);
            FIELD(pr_v1)
          }
        }
      }
...
VARIANT_TAG(binary_archive, cryptonote::txin_aimonit_key, 0x2);
...
VARIANT_TAG(binary_archive, cryptonote::txout_aimonit_tagged_key, 0x2);
